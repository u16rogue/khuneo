...
auto khuneo::xxh::hash32(const char * buffer, int length, unsigned int seed) -> xxh32
{
	...
	for (; pos + 16 <= length; pos += 16)
		for (auto & _v : v)
			_v = _khuneo_xxh_rotl32(_v + *reinterpret_cast<const xxh32 *>(buffer + pos + ((&_v - v) * 4)) * p[1], 13) * p[0];
	...
	for (; length - pos >= 4; pos += 4)
		h = _khuneo_xxh_rotl32((h + *reinterpret_cast<const xxh32 *>(buffer + pos) * p[2]), 17) * p[3];

	for (; length - pos > 0; ++pos)
		h = _khuneo_xxh_rotl32((h + *reinterpret_cast<const unsigned char *>(buffer + pos) * p[4]), 11) * p[0];
