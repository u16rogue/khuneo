...
enum kh_LexerResponse kh_ll_lexer_identifier_to_keyword(const kh_U8Char * const chunk, const kh_U8StringSize chunk_range, struct kh_LexerDescribeResult * const described) {
  ...
  if (keyword != KH_TOKEN_KEYWORD_INVALID) {
    described->keyword.type = keyword;
...
enum kh_LexerResponse kh_lexer_gobble_step(struct kh_LexerContext * ctx) {
  KH_UNUSED(ctx);
  return KH_LEXER_RES_FAIL;
}

enum kh_LexerResponse kh_lexer_gobble_stop(struct kh_LexerContext * ctx) {
  KH_UNUSED(ctx);
  return KH_LEXER_RES_FAIL;
}

enum kh_LexerResponse kh_lexer_gobble_start(struct kh_LexerContext * ctx,struct kh_U8StringView code) {
  KH_UNUSED(ctx);
  KH_UNUSED(code);
  return KH_LEXER_RES_FAIL;
}
